...
## ⏭️ ค้าง/เสนอต่อ
- **‼️ ไฟล์ 60-กะจื่อ ขาดกิ่ง 辰 ทั้งหมด (จริงมี 55 ไม่ใช่ 60):** "60-กะจื่อ" = `knownlage/ลักษณะนิสัย60แบบ_ราศีบน-ล่าง_12เซี่ยงแซ.txt` (`PERSONALITY_FILE` → บท1/บท5). กิ่ง 11 ตัวมีครบ (ตัวละ 10) แต่ **辰 (มะโรง) = 0** → ขาด 5 combos: 甲辰/丙辰/戊辰/庚辰/壬辰. ตรวจ **xlsx ต้นฉบับทั้ง 2 ที่ (knownlage/+docs/) ก็ไม่มี 辰** → ต้นฉบับเองขาด (ไม่ใช่ extraction ตก). กระทบดวงหลักวันกิ่ง辰 — ใน 6 เคส YLC คือ **ชัยธรณ์(壬辰) + กัญญารัตน์(甲辰)** บท1 ขาดคำอ่านราศีล่าง/เซียงแซ. ตอนนี้ `buildPersonalityReading` มี **fallback ระดับก้าน** คุมนิสัยพื้นฐานไว้แล้ว แต่เนื้อราศีล่าง辰 เติมไม่ได้จนกว่าจะได้เนื้อหา 5 combos จากต้นฉบับเดิม → **ต้องขอเนื้อหา 辰 จากซินแส/เจ้าของไฟล์ แล้วเติมเข้า .txt (และ xlsx)**
- เทียบเนื้อหาบทอื่น (2/3/5...) กับ YLC ครบ 6 เคส
- ถ้าต้องการ LLM นิ่ง 100% ทุกดวง → อัปเกรดโมเดลจาก flash-lite
- แก้ 14 fail (A1 skip-if-missing corpus + B1 await effect home-page)
