 ...
 */
export function NewdataReadingWorkspace2() {
  ...
  useEffect(() => {
    try {
      setDeviceLabel(localStorage.getItem("newdata-reading2:device-label") ?? "");
    } catch {
      /* localStorage ปิด */
    }
  }, []);
  const updateDeviceLabel = useCallback((value: string) => {
    ...
    try {
      localStorage.setItem("newdata-reading2:device-label", value);
    } catch {
      /* ignore */
    }
  }, []);
  ...
  const runReading = useCallback(
    async (input: BirthInput, editsOverride?: Edits) => {
      ...
      try {
        ...
        setData(body);
        const key = `newdata-reading2:${input.birthDate}:${input.birthTime}:${input.gender}`;
        setStorageKey(key);
  ...
  const predictChapterAi = useCallback(
    async (ch: ChapterView, mode: "compose" | "refine" = "compose"): Promise<boolean> => {
      ...
      try {
        const res = await fetch("/api/reading/newdata-reading2/llm", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            topicId: ch.id,
  ...
  const predictAllAi = useCallback(async () => {
    // v2: เขียนทุกบท (รวมบทที่ NewData ว่าง — persona จะเล่ากว้าง ๆ จากดิถี/กำลังแบบไม่ฟันธง)
    ...
    <div className="newdata-reading">
      ...
      >
        ...
        {data && (
          ...
          >
            {aiAll ? "💗 Louise Hay กำลังเขียน…" : "💗 เขียนทั้ง 15 บทด้วย Louise Hay"}
          </button>
        )}
        {editedCount > 0 && (
                ...
                <div className="newdata-reading__boxes no-print">
                  ...
                  <div className="newdata-reading__box-actions">
                    ...
                    <button
                      type="button"
                      className="newdata-reading__btn newdata-reading__btn--ai"
                      disabled={aiBusy[ch.id] || aiAll}
                      onClick={() => void predictChapterAi(ch, "compose")}
                      title={
                        ch.hasContent
                          ? "AI โค้ชฮีลใจ (Louise Hay) เขียนบทนี้ด้วยน้ำเสียงอบอุ่น อิงข้อมูลดวง — ทับกล่อง แต่คืนค่าได้"
                          : "บทนี้ยังไม่มีข้อมูล NewData — AI จะเล่ากว้าง ๆ จากดิถี/กำลังแบบไม่ฟันธง (ทับกล่อง คืนค่าได้)"
                      }
                    >
                      {aiBusy[ch.id] ? "💗 Louise Hay กำลังเขียน…" : "💗 เขียนด้วย Louise Hay"}
                    </button>
                    <button
                      type="button"
