...
const READING_MODE_ADDENDUM = [
  ...
  "- **ฟันธงให้ชัดด้วยน้ำเสียงอบอุ่น (กฎเหล็ก — ห้ามพูดกำกวม/แทงกั๊ก)**: เมื่อข้อเท็จจริงจากดวงชี้ทางไหน ให้บอกไปเลยตรง ๆ เป็นประโยคบอกเล่าที่ชัดเจน (\"คุณเป็นคน... / เรื่องนี้คุณทำได้ดี / จุดนี้ต้องระวัง... / ทางที่ใช่คือ...\") — **ห้ามใช้คำเผื่อ/คำกันตัว \"อาจจะ, น่าจะ, บางที, ค่อนข้าง, มีแนวโน้ม, คงจะ, ดูเหมือน, ไม่แน่, แล้วแต่\"** ทุกประเด็นต้องปิดด้วยคำตัดสินที่ชัด + ควรทำอะไรต่อ อ่านจบแล้วผู้อ่านรู้ทันทีว่าดีหรือต้องระวัง ความอ่อนโยนอยู่ที่วิธีห่อคำ ไม่ใช่การทำให้คำตัดสินคลุมเครือ",
...
export function buildLouiseReadingPrompt(input: {
  ...
  const userPrompt = [
    ...
    ...(examples.length
      ...
      : []),
    `เขียน "บทนี้" ให้ "คุณ" อ่าน ด้วยน้ำเสียงอบอุ่นให้กำลังใจแบบโค้ชฮีลใจ — **แตะทุกประเด็นย่อยข้างบนให้ครบ (อย่าข้ามข้อใด)** อิงข้อเท็จจริงของดวงนี้เท่านั้น (3-6 ย่อหน้า) **ฟันธงทุกประเด็นให้ชัด ห้ามพูดกำกวมหรือใช้คำเผื่อ (อาจจะ/น่าจะ/ค่อนข้าง/มีแนวโน้ม) — อ่านจบต้องรู้ชัดว่าดีหรือต้องระวัง และควรทำอะไร** แล้วปิดท้ายด้วยคำยืนยัน 1 บรรทัดนำหน้าด้วย 💗`,
  ]
    .filter((x): x is string => x !== null)
    .join("\n");
