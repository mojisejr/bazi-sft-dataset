...
const READING_MODE_ADDENDUM = [
  ...
  "- ตอนนี้คุณกำลังเขียน \"บทหนึ่ง\" ของรายงานอ่านดวงเต็ม ให้เขียนเป็นเนื้อความอบอุ่นไหลลื่น เต็มอิ่ม ราว 4-7 ย่อหน้า (ไม่ใช่แค่ 2-3 ประโยค)",
  "- **ครบทุกมุมสำคัญที่สุด: ต้องแตะ \"ทุกประเด็นย่อย\" ที่ระบุให้ครบ ห้ามข้ามข้อใด และห้ามตัดเนื้อหาทิ้ง** — เล่าแต่ละประเด็นให้มีรายละเอียดพอเข้าใจและใช้ได้จริง (อย่ารวบสั้นจนเหลือประโยคเดียว) เก็บใจความสำคัญจากข้อเท็จจริงที่แนบมาให้ครบ อย่ายอมตัดเพื่อความสั้น",
  "- **จัดบรรทัดให้อ่านง่าย สบายตา**: แบ่งเป็นย่อหน้าสั้น ๆ (ย่อหน้าละ 2-4 ประโยค โฟกัส 1 ประเด็น) **คั่นระหว่างย่อหน้าด้วยบรรทัดว่าง 1 บรรทัดเสมอ** ห้ามเขียนติดกันยาวเป็นพืดก้อนเดียว — ให้ผู้อ่านกวาดสายตาแล้วแยกประเด็นได้ทันที",
  ...
  "- **โหมดอ่านรายบทนี้ ห้ามลงท้ายประโยคด้วยคำลงท้ายเพศหญิง \"ค่ะ/นะคะ/ค่า/จ้ะ\"** (override กฎคำลงท้ายในเปอร์โซนาด้านบน) — เขียนเป็นน้ำเสียงเล่าเรื่องเป็นกลาง อบอุ่นได้โดยไม่ต้องมีคำลงท้าย",
...
export function buildLouiseReadingPrompt(input: {
  ...
  const userPrompt = [
    ...
    ...(examples.length
      ...
      : []),
    `เขียน "บทนี้" ให้ "คุณ" อ่าน ด้วยน้ำเสียงอบอุ่นให้กำลังใจแบบโค้ชฮีลใจ — **แตะทุกประเด็นย่อยข้างบนให้ครบ (อย่าข้ามข้อใด) เล่าเต็มอิ่มไม่ตัดเนื้อหาทิ้ง (4-7 ย่อหน้า)** อิงข้อเท็จจริงของดวงนี้เท่านั้น **แบ่งย่อหน้าสั้น ๆ คั่นด้วยบรรทัดว่างให้อ่านง่าย** **ฟันธงทุกประเด็นให้ชัด ห้ามพูดกำกวมหรือใช้คำเผื่อ (อาจจะ/น่าจะ/ค่อนข้าง/มีแนวโน้ม)** **ห้ามลงท้ายประโยคด้วย ค่ะ/นะคะ (เขียนน้ำเสียงเล่าเรื่องเป็นกลาง)** แล้วปิดท้ายด้วยคำยืนยัน 1 บรรทัดนำหน้าด้วย 💗`,
  ]
    .filter((x): x is string => x !== null)
    .join("\n");
