...
const READING_MODE_ADDENDUM = [
  ...
  "- **ยึด \"ข้อเท็จจริงจากดวง\" ที่แนบเป็นแกนเสมอ** — ห้ามแต่งธาตุ/เสา/เชี่ยงแซ/ตัวเลขที่ไม่มีในข้อมูล ถ้าส่วนไหนไม่มีข้อมูลให้พูดกว้าง ๆ อย่างซื่อสัตย์แบบไม่ฟันธง (น่าจะ/มีแนวโน้ม) ไม่เดาให้ดูดีเกินจริง",
  "- **อย่ายกตัวเลขความน่าจะเป็น/เปอร์เซ็นต์ดิบมาอ้าง** (เช่น \"โอกาสมีคู่ 20-40%\") — แปลงเป็นเชิงคุณภาพอย่างอ่อนโยน (มีแนวโน้มยาก/ปานกลาง/เปิดกว้าง) เปิดพื้นที่ให้ความหวัง ไม่ตีตราชี้ชะตาด้วยตัวเลข",
...
function factsExcerpt(boxes: LouiseReadingBox[]): string {
  ...
  return lines.length ? lines.join("\n\n") : "(บทนี้ยังไม่มีข้อเท็จจริงเฉพาะเจาะจงจากดวง — เขียนกว้าง ๆ อย่างอบอุ่นจากดิถี/กำลังที่ให้ อย่าเดารายละเอียดที่ไม่มี)";
}
