...
export type GateInfo = {
  ...
  meaning: string | null;
  /** คีย์เวิร์ดของประตู 八門 (เหมือน spirit keywords ของ 十神) — โชว์เป็นลิสต์ "8 ประตู · คีย์เวิร์ด" ใน FE */
  keywords: string[];
...
export type MonthInfo = {
  ...
  /** ทิศลาภเดือน */
  lapDir: string | null;
  /** ทิศอสูรเดือน (三煞 ตามกิ่งเดือน) = "ทิศร้าย" ประจำเดือน */
  asuraDir: string | null;
  /** ทิศ 八神 ระดับเดือน [ตัวอักษร, ทิศ] (สกัดจากปฏิทินจริง) */
  spiritDirs: [string, string][] | null;
  /** คี้มึ้งเดือน (奇門) = ก้านสวรรค์ของเสาเดือน เช่น 丙/丁 (จากเอกสารซินแส) */
  kimeng?: string | null;
  /** ตาราง 8 ประตูประจำเดือน (奇門) → ทิศ + เทพ (จากคี้มึ้งเดือน) — โชว์เป็นกริดในการ์ด "ดวงประจำเดือน" */
  gates?: GateInfo[] | null;
};
...
export type YearInfo = {
  ...
  spiritDirs: [string, string][] | null;
  /** คี้มึ้งปี (奇門) = ก้านสวรรค์ของเสาปี เช่น 丙 (จากเอกสารซินแส) */
  kimeng?: string | null;
  /** ตาราง 8 ประตูประจำปี (奇門) → ทิศ + เทพ (จากคี้มึ้งปี) — โชว์เป็นกริดในการ์ด "ดวงประจำปี" */
  gates?: GateInfo[] | null;
};
