 ...
 *  แต่ต้องติดป้ายเตือน + ไม่ฟันธง กันมโนกลับขั้ว (คำอ่านจริงซินแสอาจตรงข้าม) */
export const GEN_MARK = `⟨ยังไม่มีข้อมูลจากตำราสำหรับกล่องนี้ — ให้ "ร่าง" คำอ่านจากสี่เสา/ดิถี/กำลังที่ให้ (หลักโหราศาสตร์จีน) เป็นจุดตั้งต้นให้ซินแสแก้ แต่ต้อง: (1) ขึ้นต้นกล่องด้วยป้าย "${AI_DRAFT_TAG}" (2) ใช้คำไม่ฟันธง (น่าจะ/มีแนวโน้ม) ไม่ยืนยันขั้วดี-ร้ายเด็ดขาด (3) อิงเฉพาะเสา/ธาตุที่มีจริง ห้ามแต่งเชี่ยงแซ/เสาที่ไม่มี⟩`;
...
export const SYSTEM_MOVES = [
  ...
  "งานของคุณมี 2 แบบต่อกล่อง: (ก) กล่องที่มีเนื้อ = \"ขัดเกลา\" ให้เป็นสำนวนซินแส โดยคงข้อเท็จจริง/ขั้วดี-ร้ายเดิมไว้ (ข) กล่องที่เนื้อเป็น \"⟨ยังไม่มีข้อมูล…⟩\" = ยังไม่มีคำอ่านจากตำรา → ให้ \"ร่าง\" คำอ่านเองจากสี่เสา/ดิถี/กำลังที่ให้ + หลักโหราศาสตร์จีน เพื่อเป็นจุดตั้งต้นให้ซินแสแก้ แต่ต้อง (1) ขึ้นต้นกล่องนั้นด้วยป้ายเตือน \"⚠️ [AI ร่าง — ยังไม่มีตำรา โปรดตรวจ]\" ให้ซินแสเห็นชัดว่าเป็นการเดา (2) เขียนแบบไม่ฟันธง (ใช้ \"น่าจะ/มีแนวโน้ม\") ไม่ยืนยันขั้วดี-ร้ายเด็ดขาด เพราะคำอ่านจริงของซินแสอาจตรงกันข้าม (เช่นตำแหน่งผู้อุปถัมภ์ที่แท้จริงกลับสร้างภาระ) (3) อิงเฉพาะเสา/ธาตุที่มีจริงในสี่เสา ห้ามแต่งเสา/ธาตุ/เชี่ยงแซที่ไม่มี",
  ...
  "- กล่อง ⟨ยังไม่มีข้อมูล⟩: ร่างได้จากสี่เสา/ดิถี/กำลังที่ให้ แต่ \"ต้อง\" ขึ้นต้นด้วยป้าย \"⚠️ [AI ร่าง — ยังไม่มีตำรา โปรดตรวจ]\" + เขียนไม่ฟันธง (น่าจะ/มีแนวโน้ม) ไม่ยืนยันดี-ร้ายเด็ดขาด; ห้ามแต่งเสา/ธาตุ/เชี่ยงแซ/อายุที่ไม่มีในดวง และห้ามยกลิสต์ (อาชีพ/สี/องค์เทพ) ขึ้นมาเอง",
