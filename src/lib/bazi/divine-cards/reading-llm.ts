/**
 * LLM "เกลาคำ" สำหรับโหมดเซียน — เอา engineProse (คำตอบ engine ตามหลักน้ำหนัก)
 * มาเรียบเรียงให้ลื่นเป็นคำทำนายเดียว โดย **ห้ามคิดคำทำนายใหม่/ห้ามแต่งเติม**
 * แค่เกลาสำนวนจากหลักการที่ให้มาเท่านั้น
 *
...
const SYSTEM_INSTRUCTION = [
  "คุณคือ \"ซินแส\" ตัวจริงที่กำลังนั่งทำนายไพ่จิตวิญญาณแดนสวรรค์ให้คนตรงหน้า — พูดด้วยน้ำเสียงคนจริง อบอุ่น มั่นใจ เหมือนเล่าให้ฟัง ไม่ใช่ AI สรุปข้อมูล",
  "",
  ...
  "",
  "ถ้ามี \"คำถาม\" จากผู้รับ (สำคัญที่สุด — ต้องตรงประเด็น อย่าตอบกว้าง เอ็ม/ซินแส 2026-09-21):",
  "- **ประโยคแรกต้องฟันธงตอบคำถามนั้นตรง ๆ ก่อนเลย** (เช่น ถามเงินก้อน→\"ช่วงนี้ยังไม่ใช่จังหวะเงินก้อน...\" · ถามคนรัก→\"เขายังไม่ตัดสินใจ...\") — **ห้ามเปิดด้วยการบรรยายไพ่หรือคำเปรียบเปรยก่อน**",
  "- ดึง \"คำทำนายรายด้าน\" ที่ตรงคำถามมาเป็นแกนหลัก (ถามเงิน→ด้านการเงิน, งาน→การงาน, รัก→ความรัก) ตอบให้ตรงเรื่องที่ถาม ไม่ใช่พูดรวม ๆ ทุกด้าน",
  "- ฟันธงชัด ทักตรงใจแบบซินแสที่รู้ใจ ไม่พูดลอย ไม่สั่งสอนศีลธรรม ไม่ปิดท้ายแบบกำปั้นทุบดิน",
  "- ถ้าถามสิ่งที่ไพ่ชี้เป๊ะไม่ได้ (เลขหวย/ตัวเลข/วันเวลาแน่นอน) → ห้ามให้ตัวเลข แต่ให้ฟันธง \"จังหวะโชค/แนวโน้ม\" ของเรื่องนั้นให้ชัด อย่าเลี่ยงด้วยการเทศนา",
  ...
  const cards = reading.slots
    .map((slot) => {
      ...
      return (
        ...
        `คำสำคัญ: ${slot.card.keywords}\n` +
        `ความหมาย: ${slot.card.prophecy}` +
        (aspects ? `\n${aspects}` : "")
      );
    })
    .join("\n\n");
...
export type DivineLlmResult = { text: string; model: string };
...
export async function polishDivineReading(
  input: {
    reading: DivineReading;
    question?: string;
    apiKey?: string;
    model?: string;
