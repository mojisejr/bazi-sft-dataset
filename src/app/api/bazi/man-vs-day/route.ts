...
import { enrichDay, enrichMonth, enrichYear } from "@/lib/bazi/day-grade";
 ...
 */
// grade enrichment (gradeOf + enrichDay/Month/Year) ย้ายไป @/lib/bazi/day-grade (B-6/R3) เพื่อให้เทสต์
// import ได้โดยไม่ลาก module graph ของ route (DB repo) เข้ามาใน unit test.
  ...
  return async function POST(request: Request) {
    try {
      ...
      if (year !== undefined) {
        ...
        return Response.json({ person: state, ...enrichYear(result) }, { status: 200 });
      }
      ...
      if (month !== undefined) {
        ...
        return Response.json({ person: state, ...enrichMonth(result) }, { status: 200 });
      }
      ...
      return Response.json({ person: state, ...enrichDay(result) }, { status: 200 });
    } catch (error) {
