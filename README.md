...
- **ปฏิทินโหราศาสตร์ (Almanac /almanac)** ปฏิทินดิถีรายวันแบบทงซู — เสาวัน/เดือน/ปี + ดิถี(day-officer) + คะแนนกำลังธาตุ + เทพประจำวัน/สีมงคล/ทิศโชคลาภ/ทิศอสูร + เวลามงคล + 8 ประตู; รองรับ **ทุกปี** (อดีต/อนาคต ผ่าน lunar-javascript) และดาวน์โหลดเป็น **Excel เลียนแบบไฟล์ต้นฉบับ** (`npm run export:almanac -- <ปีพ.ศ.>`). ตาราง lookup สกัดจาก `knownlage/ManvsDay/` ด้วย `npm run extract:almanac` (เดือน ส.ค.–ธ.ค. ที่ไม่มีในไฟล์ต้นฉบับ ใช้โมเดลฤดูประมาณค่ากำลัง — ระบุ `(ประมาณ)`)
