...
- **(2026-06-13)** **MERGE `pdf-dev` → `chat/bazi-connect`** (#bazi-pdf-dev-chat-merge): รวม engine ล่าสุดของ `pdf-dev` (20 commits: domain-power, knowledge CMS/override, substitution rules → DB, WYSIWYG, phone-reading) เข้ากับ chat grounding lane. conflict เดียวคือ map ไฟล์นี้ (union resolved). chat lane ไม่เคยแตะ `/api/reading/topic` → หลัง merge ได้ reading route ที่ enhanced (knowledge overlay + DB substitution rules + `domainPower` optional) มาอัตโนมัติ โดยไม่ต้องแก้ฝั่ง chat
