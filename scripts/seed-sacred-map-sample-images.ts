...
/** name (ตรงกับที่ seed) → { slug (object key), url รูป Wikimedia (330px) } */
const SAMPLES: Array<{ name: string; slug: string; url: string }> = [
  {
...
async function main() {
  ...
  for (const s of SAMPLES) {
    ...
    if (row.image_url && !FORCE) {
      console.log(`skip (มีรูปแล้ว): ${s.name}`);
      continue;
    }

    const res = await fetch(s.url, { headers: { "user-agent": "mumate-sacred-map-seed/1.0" } });
    if (!res.ok) {
      console.log(`skip (โหลดรูปไม่ได้ ${res.status}): ${s.name}`);
      continue;
    }
