/**
 * Apply migration: bazi_newdata_reading.status (additive, idempotent)
 * Usage: npm run db:apply:newdata-reading-status
 */
...
async function main() {
  const sqlPath = path.resolve(process.cwd(), "drizzle/0027_newdata_reading_status.sql");
