...
describe("buildOpenWebUiGeminiPromptPayload — ตอบจากไพ่เซียมซีเคี้ยงคุง (hasCardReadingData)", () => {
  test("ดวงตอบไม่ได้ → ตอบจากไพ่ที่จั่ว ห้ามขึ้นดวง ไม่ชวนไปเมนู และไม่มี bazi bypass ปน", () => {
    ...
    // ซินแสนุ้ย 2026-09-20: บอกว่ามีการทำนายให้ โดย "สุ่มคำเปิด" (เสี่ยงทาย/ทำนาย/เพ่งจิต/คำนวณ/จับยาม/
    // ตาที่สาม) ต่อข้อความ จะได้ไม่เบื่อ — แต่ห้ามระบุว่าเป็น "ไพ่" (แชทข้อความ บอกไพ่แล้วคนจะเอ๊ะว่ารู้ได้ไง)
    expect(payload.userPrompt).toContain("ต้องเปิดคำตอบด้วยคำทำนองว่า");
    expect(payload.userPrompt).toMatch(/คำทำนองว่า "(เสี่ยงทาย|ขอทำนาย|ขอเพ่งจิต|ขอคำนวณ|ขอจับยาม|ขอใช้ตาที่สาม)"/);
